...
def parameter_grid_search_fit(save_file,trainX, trainY, learning_rate = [0.01,0.001,0.0001],optimizer = ['adam', 'RMSprop'],num_layers = [1,2,3],num_neurons = [100,200,400,800],model_nr=0):
    ...
    fit_save_file = "model"+str(model_nr)+".keras"
    fit_LSTM(trainX,trainY,fit_save_file,num_neurons=num_neurons,num_layers=num_layers,learning_rate=learning_rate,optimizer = optimizer)

def parameter_grid_search_prediction(model_path,save_name,testX, testY, learning_rate = [0.01,0.001,0.0001],optimizer = ['adam', 'RMSprop'],num_layers = [1,2,3],num_neurons = [100,200,400,800],model_nr=0):
    """_summary_
    ...
    prediction_save_file = model_path + "model"+str(model_nr)+".keras"
    ...
    row = {'Model number': model_nr+1, 'MSE': mse1, 'R-squared': R2}
    result_row = {"Model": model_nr+1,'Actual':testY,'Predicted':predLSTM}
...
result_row_list = []
for i in range(96):
    row,result_row = parameter_grid_search_prediction(save_file,save_name,testX,testY[:,i,0],learning_rate=learning_rate,optimizer=optimizer,num_layers=num_layers,num_neurons=num_neurons,model_nr=i)
...
diffModelData.to_pickle("96ModelData.pkl")
#RMSprop Adam
